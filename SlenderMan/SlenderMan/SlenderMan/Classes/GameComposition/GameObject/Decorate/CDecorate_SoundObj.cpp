...
void CDecorate_SoundObj::preDraw(void)
{
	...
	auto v_mat = m_stParameters.m_pCamera->getViewMatrix();
	m_pEffect->SetMatrix("g_stViewMatrix", &v_mat);
	auto p_mat = m_stParameters.m_pCamera->getProjectionMatrix();
	m_pEffect->SetMatrix("g_stProjectionMatrix", &p_mat);
	
	auto lightDirection = D3DXVECTOR4(m_stParameters.m_pDirectional->getForwardDirection(), 1.0f);
	m_pEffect->SetVector("g_stLightDirection", &lightDirection);
	auto viewPosition = D3DXVECTOR4(m_stParameters.m_pCamera->getPosition(), 1.0f);
	m_pEffect->SetVector("g_stViewPosition", &viewPosition);
