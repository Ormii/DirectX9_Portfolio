...
void CStage::load(CTerrainObject::STParameters a_stParameters, std::string m_oObjPacketListFilePath,bool reloaded)
{

	if (m_pTerrainObj != nullptr&&!reloaded)
		return;
...
void CStage::draw()
{
	...
	if (!m_bIsMaptool)
	{
		...
		auto vec = D3DXVECTOR4(0.0f, 0.0f, 1.0f, 1.0f);
		FIND_RENDERTARGET("OutlineRenderTarget")->m_pOutlineBlurEffect->SetVector("g_stOutlineColor", &vec);
