...
{
	SAFE_DELETE_ARRAY(m_stParameters.m_pSpotLight);
	SAFE_DELETE_ARRAY(m_stParameters.m_pPointLight);
	destroy();
}
		...
		if (SUCCEEDED(pIndexBuffer->Lock(0, (m_cxDIB - 1)*(m_czDIB - 1) * 2 * sizeof(DWORD) * 3, (void**)&pIndices, 0)))
		{
			for (int i = 0; i < m_nTriangles * 3; i += 6)
			{
					...
					&ray.m_stOrigin, &ray.m_stDirection, &fu, &fv, NULL))
				{
					...
					D3DXVECTOR3 du;
					auto vec = pVertices[pIndices[i + 1]].m_stPosition - pVertices[pIndices[i]].m_stPosition;
					D3DXVec3Normalize(&du,&vec);
					du *= fu;

					D3DXVECTOR3 dv;
					vec = pVertices[pIndices[i + 2]].m_stPosition - pVertices[pIndices[i]].m_stPosition;
					D3DXVec3Normalize(&dv, &vec);
					dv *= fv;
					...
					&ray.m_stOrigin, &ray.m_stDirection, &fu, &fv, NULL))
				{
					...
					D3DXVECTOR3 du;
					auto vec = pVertices[pIndices[i + 5]].m_stPosition - pVertices[pIndices[i + 3]].m_stPosition;
					D3DXVec3Normalize(&du, &vec);
					du *= fu;

					D3DXVECTOR3 dv;
					vec = pVertices[pIndices[i + 4]].m_stPosition - pVertices[pIndices[i + 3]].m_stPosition;
					D3DXVec3Normalize(&dv, &vec);
					dv *= fv;
